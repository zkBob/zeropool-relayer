...
async function createDepositPermittable(acc: UserAccount, amount: string, from: string): Promise<[TransactionData, string]> {
...
async function createFlow(acc: UserAccount, { accounts, flow }: Flow): Promise<FlowOutput> {
  ...
  for (let [i, item] of flow.entries()) {
    ...
    if ('from' in item) {
      ...
      depositSignature = packSignature(createSignature(
        {
